//jQuery(document).ready(function(){		
		...
		/*if(jQuery("#cerdapesochart").length > 0){
			var plot = jQuery.plot(jQuery("#cerdapesochart"),
			...
			jQuery("#cerdapesochart").bind("plothover", function (event, pos, item) {
			...
			jQuery("#cerdapesochart").bind("plotclick", function (event, pos, item) {
				if (item) {
		...
		/*if(jQuery("#bargraph2").length > 0){
			var d1 = [];
			for (var i = 0; i <= 10; i += 1)
				d1.push([i, parseInt(Math.random() * 20)]);
				
			var stack = 0, bars = true, lines = false, steps = false;
			jQuery.plot(jQuery("#bargraph2"), [ d1 ], {
				series: {
					stack: stack,
					lines: { show: lines, fill: true, steps: steps },
					bars: { show: bars, barWidth: 0.6 }
		...
		if(jQuery("#chartplace2").length > 0){
		...
		if(jQuery("#updateInterval").length > 0){
